...
public class Main {
    public static void main(String[] args) {
        ...
        List<Order> delivered = orders.stream().filter(order -> order.getDeliveryDate().isPresent()).collect(Collectors.toList());

        List<Order> notDelivered = orders.stream().filter(order -> !order.getDeliveryDate().isPresent()).collect(Collectors.toList());
        ...
        delivered.forEach(order -> System.out.println());
    }
}
